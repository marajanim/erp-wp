...
class JESP_ERP_Database
{
    ...
    public static function paginate($table, $args = array())
    {
        ...
        if (!empty($args['search_col']) && !empty($args['search_val'])) {
            // Whitelist allowed search columns to prevent SQL injection.
            $allowed_search_cols = array('customer_name', 'email', 'phone', 'title', 'invoice_number', 'category', 'notes');
            if (in_array($args['search_col'], $allowed_search_cols, true)) {
